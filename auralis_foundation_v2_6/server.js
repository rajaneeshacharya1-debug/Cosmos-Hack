...
function defaultDb(){
  return {
    meta: { version: '2.6', createdAt: now() },
...
function loadDb(){
  try {
    ...
    db.meta = db.meta || { version: '2.6', createdAt: now() };
    db.users = db.users || [];
...
async function handleApi(req, res, u){
  ...
  if (u.pathname === '/api/health') return json(res, { ok:true, version:'2.6', port:PORT, ts:now() });
...
http.createServer(async (req,res) => {
  ...
  console.log(`Auralis Beacon V2.6 running at http://localhost:${PORT}`);

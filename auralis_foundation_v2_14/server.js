...
function defaultDb(){
  return {
    meta: { version: '2.14', createdAt: now() },
...
function loadDb(){
  try {
    ...
    db.meta = db.meta || { version: '2.14', createdAt: now() };
    db.users = db.users || [];
...
async function handleApi(req, res, u){
  ...
  if (u.pathname === '/api/health') return json(res, { ok:true, version:'2.14', port:PORT, ts:now() });
...
http.createServer(async (req,res) => {
  ...
  console.log(`Auralis Beacon V2.14 running at http://localhost:${PORT}`);

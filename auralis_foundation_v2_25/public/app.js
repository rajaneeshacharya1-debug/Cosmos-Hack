...
function focusTrackerDemo(selector,title,step,progress,hold=1700,action=null){
  return new Promise(resolve=>{
    let el=typeof selector==='string'?document.querySelector(selector):selector;
    if(el){focusAutoDemoElement(el);}
    trackerDemoPanel(title,step,progress);
    if(action)setTimeout(action,350);
    autoDemoTimer=setTimeout(resolve,hold);
  });
}
async function runTrackerShowcase(){
  ...
  await focusTrackerDemo('.device-panel','Tracker · Selected protected phone','01/10',10,1700);
  await focusTrackerDemo('.track-stat-grid','Tracker · Battery, network and coordinates','02/10',20,1900);
  await focusTrackerDemo('#mapCard','Tracker · Live location map','03/10',30,2100);
  await focusTrackerDemo('#mapFullscreenBtn','Tracker · Entering fullscreen map','04/10',40,5600,()=>toggleMapFullscreen(false));
  await focusTrackerDemo('#mapFullscreenBtn','Tracker · Exiting fullscreen map','05/10',50,1800,()=>toggleMapFullscreen(true));
  await new Promise(r=>setTimeout(r,420));
  await focusTrackerDemo('.command-panel','Tracker · Recovery command panel','06/10',62,1800);
  await focusTrackerDemo('.ring-command-pair','Tracker · Ring and Stop Ring controls','07/10',72,2200);
  await focusTrackerDemo('.mode-card','Tracker · Tracking mode controls','08/10',82,1600);
  await focusTrackerDemo('.primary-control.danger','Tracker · Stop Recovery control','09/10',90,1600);
  await focusTrackerDemo('.command-panel details.queue','Tracker · Command queue lifecycle','10/10',100,2600);
  stopAutoDemo();

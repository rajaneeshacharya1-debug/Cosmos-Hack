...
async function runTrackerShowcase(){
  try{
    ...
    await focusTrackerDemo('.device-panel,.selected-device-card,.track-v21 .panel:first-child','Tracker · Protected phone state','01/12',8,1700);
    await focusTrackerDemo('.track-stat-grid,.mini-grid','Tracker · Battery, network and coordinates','02/12',16,2100);
    await focusTrackerDemo('#mapCard,.map-card','Tracker · Live location map','03/12',24,2200);

    await focusTrackerDemo('#mapFullscreenBtn','Tracker · Opening fullscreen map','04/12',32,6500,()=>{
      if(typeof toggleMapFullscreen==='function')toggleMapFullscreen(false);
    });
    await wait(500);
    await focusTrackerDemo('#mapFullscreenBtn','Tracker · Closing fullscreen map','05/12',42,2200,()=>{
      if(typeof toggleMapFullscreen==='function')toggleMapFullscreen(true);
    });
    await wait(700);

    await focusTrackerDemo('.command-panel,.track-v21 .panel:last-child','Tracker · Right-side recovery controls','06/12',52,1800);
    await focusTrackerDemo('.ring-command-pair,[data-cmd="RING"]','Tracker · Ring / Stop Ring controls','07/12',60,1700);

    await showcaseSendCommand('RING','Tracker · Ring command sent, waiting for phone sound','08/12',70,8500);
    await showcaseSendCommand('STOP_RING','Tracker · Stop Ring command sent, waiting for silence','09/12',80,7500);

    await focusTrackerDemo('.mode-card,.mode-grid','Tracker · Tracking mode controls','10/12',88,1700);
    await focusTrackerDemo('[data-cmd="STOP_RECOVERY"],.primary-control.danger','Tracker · Stop Recovery control','11/12',94,1600);
    await focusTrackerDemo('details.queue,.command-panel details,#commands','Tracker · Command queue lifecycle','12/12',100,3200);

...
function createDeviceForUser(user, body={}){
  const id = user.id === 'user-demo' ? DEVICE_ID : ('device-' + crypto.randomBytes(4).toString('hex'));
  ...
  return {
    ...
    name: String(body.name || (user.id === 'user-demo' ? 'OPPO F19 Pro' : 'My Android Phone')).slice(0,80),
    model: String(body.model || (user.id === 'user-demo' ? 'OPPO F19 Pro' : 'Android Device')).slice(0,80),
    token: user.id === 'user-demo' ? DEVICE_TOKEN : ('auralis-user-token-' + crypto.randomBytes(8).toString('hex')),
    mode: 'STANDBY',
    battery: null,
    network: 'offline',
    lat: null,
...
function ownsDevice(user, d){
  return !!(user && d && d.ownerId === user.id);
}

function defaultDb(){
  return {
    meta: { version: '2.29', createdAt: now() },
    users: [{ id: 'user-demo', name: 'Auralis Owner', email: 'owner@auralis.local', role:'owner', passwordHash: sha('Auralis@4729') }],
    sessions: {},
...
function loadDb(){
  try {
    ...
    db.meta = db.meta || { version: '2.29', createdAt: now() };
    db.users = db.users || [];
    ...
    if (!d) db.devices.push(defaultDevice());
      ...
      d.ownerEmail = d.ownerEmail || 'owner@auralis.local';
...
async function handleApi(req, res, u){
  ...
  if (u.pathname === '/api/health') return json(res, { ok:true, version:'2.29', port:PORT, ts:now() });
  ...
  if (u.pathname === '/api/login' && req.method === 'POST') {
    ...
    save();
    return json(res, { ok:true, session:sid, user:{ id:user.id, name:user.name, email:user.email, role:user.role || 'user' } });
  }
  ...
  if (u.pathname === '/api/register' && req.method === 'POST') {
    ...
    const user = { id:'user-' + crypto.randomBytes(5).toString('hex'), name:String(b.name || 'Auralis User').slice(0,80), email, role:'user', passwordHash:sha(password), createdAt:now() };
    db.users.push(user); save();
    return json(res, { ok:true, user:{ id:user.id, name:user.name, email:user.email, role:user.role || 'user' } });
  }
  ...
  if (u.pathname === '/api/devices') {
    const user = userFromSession(req);
    if (!user) return json(res, { ok:false, error:'AUTH_REQUIRED' }, 401);
    const owned = db.devices.filter(d => d.ownerId === user.id).map(publicDevice);
    return json(res, { ok:true, user:{ id:user.id, name:user.name, email:user.email, role:user.role || 'user' }, devices:owned, emptyState: owned.length === 0 });
  }
  ...
  if (u.pathname === '/api/register-device' && req.method === 'POST') {
    ...
    // Owner keeps the real OPPO demo device. Other accounts get their own clean protected-device record.
    let d;
    if (user.id === 'user-demo') {
      d = db.devices.find(x => x.id === DEVICE_ID);
      if (!d) { d = defaultDevice(); db.devices.push(d); }
      d.ownerId = user.id;
      d.ownerEmail = user.email;
      d.name = String(b.name || d.name || 'OPPO F19 Pro').slice(0,80);
      d.model = String(b.model || d.model || 'OPPO F19 Pro').slice(0,80);
      d.token = DEVICE_TOKEN;
    } else {
      d = db.devices.find(x => x.ownerId === user.id);
      if (!d) {
        d = createDeviceForUser(user, b);
        db.devices.push(d);
      } else {
        d.name = String(b.name || d.name || 'My Android Phone').slice(0,80);
        d.model = String(b.model || d.model || 'Android Device').slice(0,80);
        d.ownerEmail = user.email;
      }
    }
    ...
    event(d, 'DEVICE_REGISTERED', { name:d.name, model:d.model, ownerEmail:user.email });
    save();
    return json(res, { ok:true, device:publicDevice(d), token:d.token, user:{ id:user.id, email:user.email } });
  }
  ...
  if (parts[0] === 'api' && parts[1] === 'device' && parts[2]) {
    ...
    if (action === 'state') {
      const user = userFromSession(req);
      if (!user) return json(res, { ok:false, error:'AUTH_REQUIRED' }, 401);
      if (!ownsDevice(user, d)) return json(res, { ok:false, error:'DEVICE_NOT_LINKED_TO_ACCOUNT' }, 403);
...
http.createServer(async (req,res) => {
  ...
  console.log(`Auralis Beacon V2.29 running at http://localhost:${PORT}`);

AURALIS BEACON FOUNDATION V2.29
...
V2.29:
- Rebuilds Auto Demo as a precise page-by-page scanner.
- Auto Demo starts at Home and visits pages in sidebar order.
- On each page it highlights elements one by one in visual order: left-to-right, then row-by-row downward.
- Adds a moving spotlight and synthetic hover/focus effect for the current element.
- Makes the Auto Demo panel compact so it does not block the UI.
- Extends hover/focus styling to cards, buttons, status panels, macro cards, architecture nodes and command lifecycle rows.

V2.29:
- Fixes login buttons not responding.
- Root cause: duplicate auto-demo JavaScript declarations in V2.22 caused a syntax error, so click bindings did not run.
- Removed duplicate old auto-demo block.
- Added app.js syntax check.
- Added robust login/quick-fill event binding and pointer-event guard for login controls.

V2.29:
- Auto Demo now scans each page by large visual template first, then elements inside that template left-to-right and row-by-row.
- Auto Demo demonstrates Ring Device, waits long enough for MacroDroid polling and sound, then demonstrates Stop Ring and waits for stop.
- Page changes now scroll content to the top, so Activity/Command History opens from the top.
- Quick fill now targets exact loginEmail/loginPassword IDs, fixing email not filling.
- Tracking battery/network/latitude/longitude boxes have icon-above text/value layout.
- Map beacon ripple is stronger and more vibrant.

V2.29:
- Adds a separate Tracker Showcase menu/page and top-bar button.
- Tracker Showcase only demonstrates the tracker page.
- It focuses selected device, battery/network/location stats, map, fullscreen map, exits fullscreen, then focuses right-side recovery controls and command queue.
- Network display formats MOBILE_DATA as Mobile / Data on two lines.
- Adds command queue animation, tracker-showcase card animations, fullscreen map glow, and stronger tracker hover effects.

V2.29:
- Fixes Tracker Showcase not starting.
- Adds robust event delegation and DOMContentLoaded binding for Tracker Showcase buttons.
- Replaces the tracker-showcase runner with a safer async flow.
- Keeps fullscreen map open long enough to observe, then exits and continues to controls/command queue.
- Adds error handling so any future failure shows an alert instead of silently doing nothing.

V2.29:
- Fixes tracker stats not updating. V2.26 called networkHtml() without defining it, which stopped render() halfway.
- Restores live rendering for last update, battery, network, latitude, longitude, map marker and command queue.
- Adds Tracker Showcase buttons robustly for the top bar, sidebar and home action card.
- Tracker Showcase now sends Ring Device, waits 8.5 seconds for MacroDroid polling/sound, then sends Stop Ring and waits 7.5 seconds.
- Keeps the fullscreen map open/exit sequence and continues to right-side controls and command queue.

V2.29:
- Tracker Showcase now first focuses all telemetry cards, then Battery, Network, Latitude and Longitude one by one.
- Tracker Showcase step count updated to 16 steps.
- Desktop preloader is slightly longer.
- Android/mobile preloader is longer and more animated, with beacon pulse, shine sweep, dashed map orbit and orbiting dots.
- Mobile spotlight positioning is more accurate: it recalculates after scrolling and clamps the highlight box to the viewport.
- Mobile auto-demo spotlight is less intrusive and better aligned.

V2.29:
- Adds multi-user demo database behaviour.
- Registration now saves new user accounts in auralis-db.json.
- Owner account still sees the OPPO F19 Pro demo device.
- New accounts login successfully but start with "No secured devices found".
- Empty-state banner and device card guide users to "Add this device".
- Registering a device for a non-owner creates a separate protected-device record instead of stealing the owner OPPO demo device.
- Dashboard commands/config are restricted to devices owned by the logged-in account.
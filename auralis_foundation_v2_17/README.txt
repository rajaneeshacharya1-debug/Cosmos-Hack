AURALIS BEACON FOUNDATION V2.17
...
V2.17:
- Adds Judge Demo page with guided hackathon presentation flow.
- Adds live demo status cards: device state, bridge poll and latest command.
- Adds clickable demo actions: reset, open tracker, ring, stop ring, stop recovery and resume.
- Adds 2-minute demo script and copy-script button.
- Adds architecture diagram inside the app.
- Adds MacroDroid Setup page with exact 5-macro stack and variable rules.
- Adds visible command lifecycle UI: PENDING -> RECEIVED -> COMPLETED.
- Corrects MacroDroid wording: dashboard STOP_RECOVERY pauses live tracking but keeps Recovery_Mode true so polling continues.
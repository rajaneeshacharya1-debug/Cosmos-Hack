...
function quickFillLogin(){
  let email=$('email')||document.querySelector('input[type="email"], input[name="email"]');
  let pass=$('password')||$('pass')||$('pwd')||document.querySelector('input[type="password"], input[name="password"]');
  if(email) email.value='owner@auralis.local';
  if(pass) pass.value='Auralis@4729';
...
function collectAutoDemoTargets(){
  ...
  if(!page)return [];
  let raw=[...page.querySelectorAll(AUTO_DEMO_SELECTOR)];
  raw=raw.filter(el=>{
  ...
  raw=uniqueElements(raw);
  raw.sort((a,b)=>{
    let ra=a.getBoundingClientRect(), rb=b.getBoundingClientRect();
    let rowA=Math.round(ra.top/42), rowB=Math.round(rb.top/42);
...
function runAutoDemo(){
  ...
  const runTarget=()=>{
    if(!autoDemoRunning)return;
    if(autoDemoTargetIndex>=autoDemoTargets.length){clearAutoDemoFocus();autoDemoPageIndex++;autoDemoTimer=setTimeout(runPage,520);return;}
    let page=AUTO_DEMO_PAGES[autoDemoPageIndex];
    ...
    autoDemoTimer=setTimeout(runTarget,1150);
  };
  runPage();
}
...
document.addEventListener('click',e=>{let go=e.target.closest('[data-go]');if(go)showPage(go.dataset.go);let nav=e.target.closest('.nav,.mobile-nav button');if(nav&&nav.dataset.page)showPage(nav.dataset.page);let cmd=e.target.closest('[data-cmd]');if(cmd)sendCommand(cmd.dataset.cmd);let view=e.target.closest('[data-view]');if(view)setTrackerView(view.dataset.view);let select=e.target.closest('[data-select-device]');if(select){currentDevice=devices.find(d=>d.id===select.dataset.selectDevice)||currentDevice;showPage('track');render()}});function bind(id,ev,fn){let el=$(id);if(el)el[ev]=fn;}bind('loginTab','onclick',()=>setAuthTab('login'));bind('registerTab','onclick',()=>setAuthTab('register'));bind('loginBtn','onclick',login);bind('regBtn','onclick',register);bind('logoutBtn','onclick',logout);bind('sidebarToggle','onclick',toggleSidebar);bind('copyUrls','onclick',copyUrls);bind('copyUrlsHome','onclick',copyUrls);bind('simMove','onclick',simulateMove);bind('setupBtn','onclick',registerDevice);bind('saveCfg','onclick',saveConfig);bind('mapFullscreenBtn','onclick',()=>toggleMapFullscreen());bind('autoDemoBtn','onclick',runAutoDemo);bind('autoDemoHomeBtn','onclick',runAutoDemo);bind('demoAutoRunBtn','onclick',runAutoDemo);bind('autoDemoStop','onclick',stopAutoDemo);bind('quickFillLoginBtn','onclick',quickFillLogin);bind('autoDemoBtn','onclick',runAutoDemo);bind('autoDemoHomeBtn','onclick',runAutoDemo);bind('demoAutoRunBtn','onclick',runAutoDemo);bind('autoDemoStop','onclick',stopAutoDemo);bind('quickFillLoginBtn','onclick',quickFillLogin);bind('demoResetBtn','onclick',resetDemoState);window.addEventListener('keydown',e=>{if(e.key==='Escape'&&document.body.classList.contains('map-fullscreen-active'))toggleMapFullscreen(true)});window.addEventListener('resize',()=>{if(map)setTimeout(()=>map.invalidateSize(),160)});restore();setInterval(refreshState,2000);

let autoDemoTimer=null, autoDemoIndex=0, autoDemoRunning=false;
const AUTO_DEMO_STEPS=[
  {page:'home',title:'Auralis Beacon',text:'One clean recovery interface for phones and laptops. The status card, tracker and command bridge are connected here.',hold:2600},
  {page:'demo',title:'Animated recovery flow',text:'SMS ignition wakes the lost phone, the server queues commands, MacroDroid polls, and the phone performs actions.',hold:3800},
  {page:'track',title:'Live tracker',text:'Battery, network, coordinates and the map update while recovery is active.',hold:3600},
  {page:'track',title:'Recovery controls',text:'Ring Device and Stop Ring are real server commands received through the phone poller.',hold:3300},
  {page:'activity',title:'Command lifecycle',text:'Commands show Pending, Received and Completed so judges can see the phone responded.',hold:3200},
  {page:'macros',title:'MacroDroid bridge',text:'Exactly five phone macros power the prototype bridge: boot, live tracking, poller, ring loop and emergency stop.',hold:3900},
  {page:'settings',title:'Bridge URLs',text:'Settings exposes the exact API URLs used by MacroDroid and the future native Android agent.',hold:3000},
  {page:'demo',title:'Ready for live judging',text:'Now run the live sequence: SMS boot, map update, Ring Device, Stop Ring, Stop Recovery and Resume.',hold:3000}
];
function quickFillLogin(){let email=$('loginEmail')||$('email')||document.querySelector('input[type="email"], input[name="email"]');let pass=$('loginPassword')||$('password')||$('pass')||$('pwd')||document.querySelector('input[type="password"], input[name="password"]');if(email)email.value='owner@auralis.local';if(pass)pass.value='Auralis@4729';let btn=$('quickFillLoginBtn');if(btn){btn.textContent='Demo login filled';setTimeout(()=>btn.textContent='Quick fill demo login',1400)}}
function setAutoOverlay(step,total){let o=$('autoDemoOverlay'),s=$('autoDemoStep'),t=$('autoDemoTitle'),p=$('autoDemoText'),bar=$('autoDemoProgress');if(!o)return;o.classList.remove('hidden');if(s)s.textContent=String(autoDemoIndex+1).padStart(2,'0')+' / '+String(total).padStart(2,'0');if(t)t.textContent=step.title;if(p)p.textContent=step.text;if(bar)bar.style.width=Math.round(((autoDemoIndex+1)/total)*100)+'%'}
function stopAutoDemo(){autoDemoRunning=false;if(autoDemoTimer)clearTimeout(autoDemoTimer);autoDemoTimer=null;let o=$('autoDemoOverlay');if(o)o.classList.add('hidden');document.body.classList.remove('auto-demo-running')}
function runAutoDemo(){if(autoDemoRunning){stopAutoDemo();return}autoDemoRunning=true;autoDemoIndex=0;document.body.classList.add('auto-demo-running');const tick=()=>{if(!autoDemoRunning)return;let step=AUTO_DEMO_STEPS[autoDemoIndex];if(!step){stopAutoDemo();return}showPage(step.page);setAutoOverlay(step,AUTO_DEMO_STEPS.length);autoDemoIndex++;autoDemoTimer=setTimeout(tick,step.hold||3000)};tick()}
